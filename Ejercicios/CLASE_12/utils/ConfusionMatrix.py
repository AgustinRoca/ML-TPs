...
class ConfusionMatrix():
    ...
    def add_result(self, real_value, predicted_value):
        for value in self.possible_values:
            if real_value in value:
                self.matrix[value][predicted_value] += 1
    ...
    def calculate_category_rates(self,value):
        ...
        other_values = np.delete(self.possible_values,np.where(self.possible_values == value))
    ...
    def calculate_metrics(self):
        ...
        for value in self.possible_values:
            metrics[value]= dict()
            TP, TN, FP, FN = self.calculate_category_rates(value)            
            metrics[value]["precision"] = self.calculate_precision(TP,FN, FP, FN)
            metrics[value]["accuracy"] = self.calculate_accuracy(TP,TN,FP,FN)
            metrics[value]["f1"] = self.calculate_F1_score(TP,FN, FP, FN)
            metrics[value]["TPrate"] = self.calculate_TP_rate(TP,FN, FP, FN)
            metrics[value]["FPrate"] = self.calculate_FP_rate(TP,FN, FP, FN)
            metrics[value]['recall'] = self.calculate_recall(TP,TN,FP,FN)
        return metrics
    ...
    def calculate_F1_score(self,TP,TN,FP,FN):
        precision = self.calculate_precision(TP,FN, FP, FN)
        recall = self.calculate_recall(TP,FN, FP, FN)
